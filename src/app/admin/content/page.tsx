...
import { useState, useEffect } from "react";import { Save, Layout, Navigation2, Info, Award, ListOrdered,
  ...
  Calendar, Plus, Trash2, ChevronDown, Upload,
} from "lucide-react";
        ...
        <div className="p-5 space-y-6">
          ...
          {activeTab === "about" && (
            <>
              ...
              <div className="border-t border-slate-100 pt-4">
                <h3 className="text-sm font-semibold text-slate-700 mb-3">Companies Worked At</h3>
                <p className="text-xs text-slate-400 mb-3">Add companies you've worked at with optional company photos.</p>
                {(content.about?.companies || []).map((company: any, i: number) => (
                  <div key={i} className="bg-slate-50 rounded-xl p-4 mb-3 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-slate-500">Company {i + 1}</span>
                      <button onClick={() => {
                    ...
                    <div className="grid sm:grid-cols-2 gap-3">
                      ...
                      <Input label="Role" value={company.role || ""}
                        onChange={(v) => {
                          const arr = [...((content.about?.companies) || [])];
                          arr[i] = { ...arr[i], role: v };
                          updateField("about", "companies", arr);
                      ...
                      <div className="sm:col-span-2">
                        <Input label="Description" value={company.description || ""}
                          onChange={(v) => {
                            const arr = [...((content.about?.companies) || [])];
